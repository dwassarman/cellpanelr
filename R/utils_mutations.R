...
volcano_plot <- function(data) {
  data %>%
    ...
    xlab(bquote(log[2]( mutant / wild-type ))) +
...
#'
#' @param data A tibble, user data merged with mutation data
...
#'
#' @return Boxplot
mut_plot_selected <- function(data, selected, response, log_scale = FALSE) {
  p <- data %>%
